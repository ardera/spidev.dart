...
abstract class SpiTransferData {
  ...
  factory SpiTransferData.fromNativeMem({
    ffi.Pointer txPointer,
    ffi.Pointer rxPointer,
    @required int length
  }) {
    return NativeMemSpiTransferData._private(
      txPointer: txPointer ?? ffi.nullptr,
      rxPointer: rxPointer ?? ffi.nullptr,
      length: length
    );
  }

  factory SpiTransferData.fromTypedData({
    Uint8List txBuf,
    Uint8List rxBuf
  }) {
    return TypedDataSpiTransferData._private(
      txBuf: txBuf,
      rxBuf: rxBuf
    );
  }

  factory SpiTransferData.fromByteLists({
    List<int> txBuf,
    List<int> rxBuf
  }) {
    return ByteListSpiTransferData._private(
      txBuf: txBuf,
      rxBuf: rxBuf
    );
  }
...
class NativeMemSpiTransferData extends SpiTransferData {
  ...
  NativeMemSpiTransferData._private({
    this.txPointer,
    this.rxPointer,
    this.length
  }) : assert(txPointer != null),
      assert(rxPointer != null),
      assert(length != null),
...
class TypedDataSpiTransferData extends SpiTransferData {
  ...
  TypedDataSpiTransferData._private({
    this.txBuf,
    this.rxBuf
  ...
  Tuple2<Map<Uint8List, ffi.Pointer>, Set<ffi.Pointer>> nativeMemFromOverlappingData(Set<Uint8List> data) {
    final start = data
      .map((v) => v.offsetInBytes)
      .reduce(min);

    final end = data
      .map((v) => v.offsetInBytes + v.lengthInBytes)
      .reduce(max);
    
    final length = end - start;

    final ptr = ffi.allocate<ffi.Uint8>(count: length);
  ...
  Tuple2<NativeMemSpiTransferData, void Function()> _toNativeMemTransferData() {
    ...
    final nativeMem = nativeMemFromByteData({
      if (rxBuf != null) rxBuf,  
      if (txBuf != null) txBuf
    });
    ...
    if (txBuf != null) {
      txPtr = nativeMem
        .item1
        .entries
        .singleWhere((element) => element.key == txBuf)
        .value;
      
      final txPtrAsBuf = txPtr
        .cast<ffi.Uint8>()
        .asTypedList(txBuf.lengthInBytes);
    ...
    if (rxBuf != null) {
      rxPtr = nativeMem
        .item1
        .entries
        .singleWhere((element) => element.key == rxBuf)
        .value;
    }

    final postTransfer = () {
      if (rxBuf != null) {
        final rxPtrAsBuf = rxPtr
          .cast<ffi.Uint8>()
          .asTypedList(rxBuf.lengthInBytes);
      ...
      nativeMem.item2.forEach((pointer) => ffi.free(pointer));
    };

    final raw = SpiTransferData.fromNativeMem(
      txPointer: txPtr,
      rxPointer: rxPtr,
      length: txBuf?.length ?? rxBuf.length
    );

    return Tuple2(raw, postTransfer);
  }
}
...
class ByteListSpiTransferData extends SpiTransferData {
  ...
  ByteListSpiTransferData._private({
    @required this.txBuf,
    @required this.rxBuf
  }) : assert(txBuf != null || rxBuf != null),
      assert((txBuf == null || rxBuf == null) || (txBuf.length == rxBuf.length));
  ...
  Tuple2<NativeMemSpiTransferData, void Function()> _toNativeMemTransferData() {
    ...
    if (txBuf != null) {
      txPointer = ffi.allocate<ffi.Uint8>(count: txBuf.length);
      final txPointerAsBuf = txPointer.cast<ffi.Uint8>().asTypedList(txBuf.length);

      txBuf.asMap().forEach((key, value) {txPointerAsBuf[key] = value;});
    }

    if (rxBuf != null) {
      rxPointer = ffi.allocate<ffi.Uint8>(count: rxBuf.length);
    }

    final postTransfer = () {
      if (rxBuf != null) {
        final rxPtrAsBuf = rxPointer.cast<ffi.Uint8>().asTypedList(rxBuf.length);
        rxPtrAsBuf.asMap().forEach((key, value) {rxBuf[key] = value;});
      }
...
class SpiTransferProperties {
  ...
  SpiTransferProperties({
    ...
    assert(delay != null && delay.inMicroseconds >= 0 && delay.inMicroseconds <= 0xFFFF),
    assert(bitsPerWord == null || (bitsPerWord >= 0 && bitsPerWord <= 32)),
    assert(doToggleCS != null),
    assert(wordDelay != null && wordDelay.inMicroseconds >= 0 && wordDelay.inMicroseconds <= 0xFF);

  int nbitsFromTransferMode(SpiTransferMode mode) {
    switch (mode) {
      case SpiTransferMode.single: return 1;
      case SpiTransferMode.dual: return 2;
      case SpiTransferMode.quad: return 4;
  ...
  await for (dynamic untypedTransfer in receivePort) {
    if (untypedTransfer == SpiTransferExecutor._CLOSE) {
      ...
      try {
        ...
        SpidevPlatformInterface._staticIoctl(
          libc,
          fd,
...
class SpiTransferExecutor {
  factory SpiTransferExecutor(int fd) {
    ...
    final executor = SpiTransferExecutor._construct(
      fd,
      isolateCompleter.future,
      toExecutorCompleter,
      fromExecutor
    );
    ...
    final isolateFuture = Isolate.spawn(
      ...
      Tuple2(fd, fromExecutor.sendPort),
  ...
  SpiTransferExecutor._construct(
    this._fd,
    this._isolateFuture,
    this._toExecutorCompleter,
    this._fromExecutor
  );

  static const _CLOSE = null;

  final int _fd;
  final Future<Isolate> _isolateFuture;
  final Completer<SendPort> _toExecutorCompleter;
  final ReceivePort _fromExecutor;
  ...
  Future<void> transfer(List<SpiTransfer<NativeMemSpiTransferData>> transfers) {
    if (canTransfer == false) {
      throw StateError(
        "Can't execute any transfers, `canTransfer` is false. "
    ...
    final pointer = ffi.allocate<spi_ioc_transfer>(count: transfers.length);
    transfers
      .asMap()
      .forEach((key, value) {
...
class SpidevPlatformInterface {
  ...
  static int _staticIoctl(LibC libc, int fd, int request, ffi.Pointer argp) {
    final result = libc.ioctlPointer(fd, request, argp.cast<ffi.Void>());
    if (result < 0) {
  ...
  int _ioctl(int fd, int request, ffi.Pointer argp)
    => _staticIoctl(libc, fd, request, argp);

  int open(String path) {
    final nativePath = ffi.Utf8.toUtf8(path);
  ...
  void setModeAndFlags(int fd, SpiMode mode, Set<SpiFlag> flags) {
    int arg = 0;
    switch (mode) {
      case SpiMode.mode0: arg |= SPI_MODE_0; break;
      case SpiMode.mode1: arg |= SPI_MODE_1; break;
      case SpiMode.mode2: arg |= SPI_MODE_2; break;
      case SpiMode.mode3: arg |= SPI_MODE_3; break;
      default: throw ArgumentError.value(mode, "mode");
    }
    for (final flag in flags) {
      switch (flag) {
        case SpiFlag.csActiveHigh: arg |= SPI_CS_HIGH; break;
        case SpiFlag.lsbFirst: arg |= SPI_LSB_FIRST; break;
        case SpiFlag.threeWire: arg |= SPI_3WIRE; break;
        case SpiFlag.loopBack: arg |= SPI_LOOP; break;
        case SpiFlag.noCs: arg |= SPI_NO_CS; break;
        case SpiFlag.ready: arg |= SPI_READY; break;
        case SpiFlag.txDual: arg |= SPI_TX_DUAL; break;
        case SpiFlag.txQuad: arg |= SPI_TX_QUAD; break;
        case SpiFlag.rxDual: arg |= SPI_RX_DUAL; break;
        case SpiFlag.rxQuad: arg |= SPI_RX_QUAD; break;
        case SpiFlag.toggleCsAfterEachWord: arg |= SPI_CS_WORD; break;
        case SpiFlag.txOctal: arg |= SPI_TX_OCTAL; break;
        case SpiFlag.rxOctal: arg |= SPI_RX_OCTAL; break;
        case SpiFlag.threeWireHighImpedanceTurnaround: arg |= SPI_3WIRE_HIZ; break;
        default: throw ArgumentError.value(flags, "flags", "Invalid SpiFlag: ${flag?.toString() ?? "null"}");
      }
    }

    final modePointer = ffi.allocate<ffi.Uint8>();
    modePointer.value = arg;
    
    try {
      _ioctl(fd, SPI_IOC_WR_MODE, modePointer);
    } finally {
      ffi.free(modePointer);
    }
  }

  Tuple2<SpiMode, Set<SpiFlag>> getModeAndFlags(int fd) {
    final nativeResultPtr = ffi.allocate<ffi.Uint8>();
    ...
    try {
      ...
      switch (nativeResult & 0x03) {
        case 0: mode = SpiMode.mode0; break;
        case 1: mode = SpiMode.mode1; break;
        case 2: mode = SpiMode.mode2; break;
        case 3: mode = SpiMode.mode3; break;
        default: throw StateError("kernel returned invalid SPI mode $nativeResult");
      }
  ...
  void setMaxSpeed(int fd, int speedHz) {
    final pointer = ffi.allocate<ffi.Uint32>();
  ...
  void setWordSize(int fd, int bitsPerWord) {
    final pointer = ffi.allocate<ffi.Uint8>();
...
class SpidevHandle {
  ...
  Future<void> transfer(List<SpiTransfer> transfers) {
    ...
    return transferNativeMem(rawTransfers)
      .whenComplete(() {
  ...
  Future<void> transferSingleNativeMem({
    ...
    return transferNativeMem([
      SpiTransfer(
        data: SpiTransferData.fromNativeMem(
          txPointer: txBuf,
          rxPointer: rxBuf,
          length: length
        ),
